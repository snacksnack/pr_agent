...
**One pipeline: Python context, routed reviewers, Python merge.** RC1-390
...
The repository context (conventions file, callers, tests; RC1-393/394) reads
the repository the same way (RC1-364): one Git Trees call for the file list,
then Contents API reads at the PR head, cached per review. Until RC1-364 the
live path pointed at an empty directory and every read came back `no such
file`, so live reviews were written from the diff alone. Both backends
implement one contract, `RepositoryAccess` (`read_text` / `grep` / `paths`;
RC1-424), so the dry-run CLI and the live path see the same repository through
the same guards. Two limits keep the live one cheap: `REMOTE_API_BUDGET`
(default 60) caps API calls per review, and a grep reads at most 30 candidate
files, changed files first. If the App token cannot read the tree, `read_text`
still works and `grep` confines itself to the PR's changed files; a search the
budget refuses altogether is reported to the reviewers as not searched, never
as "no matches".
...
app/
  ...
  agent/
    ...
    local_repository.py   # ...from a checkout on disk (RC1-109)
    github_repository.py  # ...from the Trees + Contents APIs at the PR head, budgeted (RC1-364)
